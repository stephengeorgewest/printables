...
type item = {
	name: string,
	MST: number,
	location: Array<location | "Steal from shop" | "Gaila">,
	character: ch[],
	note?: string
};
...
type shoe = item & { defense: number, agility: number, attack: number };
const footwear: shoe[] = [
...
function setCharacterSelectOptions(selectedCharacter?: ch) {
...
function setMaxLocation(l: location) {
...
let selectedCharacters: ch[] = [];
let selectedLocations: location[] = [];
function setCharacter(character?: ch | null | undefined) {
...
function setLocations() {
	...
	locationsCheckboxes.childNodes.forEach(span => {
		const checkbox = span.firstChild as HTMLInputElement
		if (checkbox.checked)
			selectedLocations.push(checkbox.value as location);
	});
	setTables();
}
function setTables() {
...
function setTable(table: string, data: item[]) {
